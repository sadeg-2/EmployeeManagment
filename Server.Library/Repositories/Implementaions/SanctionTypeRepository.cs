...
namespace Server.Library.Repositories.Implementaions
{
    public class SanctionTypeRepository(AppDbContext appDbContext) : IGenericRepositoryInterface<SanctionType>
    {
        ...
        {
            var item = await appDbContext.SanctionTypes.FirstOrDefaultAsync(x => x.Id == id);
        ...
        {
            if (!await CheckName(item.Name!)) return new(false, "Sanction Type Already added");
        ...
        {
            var sanctionType = await appDbContext.SanctionTypes.FirstOrDefaultAsync(x => x.Name!.ToLower().Equals(name.ToLower()));
            return sanctionType is null;
        }
